...
void Boot_HandleFlash(UART_HandleTypeDef *huart)
{
    ...
    /* Step 3: Receive IV (12 bytes) */
    uint8_t iv[IV_SIZE];
    UART_ReadExact(huart,iv, IV_SIZE);
    Boot_SendAck(huart);
    /* Step 4: Receive Tag (16 bytes) */
    uint8_t tag[TAG_SIZE];
    UART_ReadExact(huart,tag, TAG_SIZE);
    Boot_SendAck(huart);
    /* Step 5: Receive original size (4 bytes) */
    uint8_t  size_buf[4];
    ...
    /* Step 6: Stream decompress from UART directly to flash */
    if (!Decrypt_DecomprToFlash(huart, original_size, APP_ADDRESS, iv, tag))
    {
    ...
    /* DEBUG */
    uint8_t dbg[4];
    Flash_Read(APP_ADDRESS, dbg, 4);
    HAL_UART_Transmit(huart, dbg, 4, 1000);
    /* Step 8: Verify signature against decompressed flashed content */
    if (!Boot_VerifySignature(APP_ADDRESS, original_size, signature))
    {
        Flash_Erase(APP_ADDRESS);
        Boot_SendErr(huart);
        return;
